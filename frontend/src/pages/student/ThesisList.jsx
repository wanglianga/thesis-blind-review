...
function StudentThesisList() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const result = await getMyThesisList({
  ...
  const columns = [
    ...
    {
      title: '学科方向',
      dataIndex: 'subjectDirectionName',
      width: 120,
    },
    {
      title: '版本',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button type="link" icon={<EyeOutlined />} onClick={() => navigate(`/student/thesis/${record.id}`)}>
            详情
          </Button>
        </Space>
      ),
  ...
  return (
    <div>
      <h2 className="page-header">我的论文</h2>
      ...
      <Card style={{ marginBottom: 16 }}>
        <Space>
          ...
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/student/thesis/submit')}>
            提交新论文
          </Button>
        </Space>
      </Card>
...
export default StudentThesisList
